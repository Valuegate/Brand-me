import { SingleQuizComponent } from "@/components/quiz/types";
...
import { MdDelete, MdAddCircle, MdAddCircleOutline } from "react-icons/md";
import { toast, ToastContainer } from "react-toastify";

const EditQuiz = () => {
  const title = useEditCourseStore((state) => state.quizzes[0].title);
  const instructions = useEditCourseStore(
    (state) => state.quizzes[0].description
  );

  const questions = useEditCourseStore((state) => state.quizzes[0].questions);
          ...
          <>
            ...
            <div className="mt-10 w-[60%] flex flex-col gap-5">
              <InputComponent
                ...
                onChange={(e) => {
                  let q = useEditCourseStore.getState().quizzes[0];
